...
To regenerate Candid files:
    delete declarations folder
    copy --delete src/declarations to end of "copy:types" in package.json (see        "copy:types:GenererateCandid")
    dfx deploy
    delete --delete src/declarations from "copy:types"
    npm start